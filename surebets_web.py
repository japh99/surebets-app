...
# Lista de claves API para rotación
API_KEYS = [
...
# Interfaz
st.title("🔎 Buscador Profesional de Surebets")
if st.button("Buscar Surebets"):
    ...
    with st.expander("📊 Consumo de APIs"):
        api_df = pd.DataFrame(list(uso.items()), columns=["API Key", "Llamadas"])
        st.dataframe(api_df)
else:
    st.info("Haz clic en el botón para comenzar la búsqueda de surebets.")
